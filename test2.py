...
def large_motor_check(noisy, sound, display, button): #---------------
    ...
    steer_motors.on(steer, speed)
    rotationValue = 0
    while (rotationValue<350):
        values_gyro = gyro_sensor.angle_and_rate
        rotationValue = values_gyro[0]
        print_display(display, 'Angle ' + str(values_gyro[0])
                      + '°, rate ' + str(values_gyro[1]) + '°/s')
        time.sleep(0.5)
    steer_motors.off()
